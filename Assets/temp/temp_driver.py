...
ST7796_RAMCTRL = const(0xB0)
ST7796_PORCTRL = const(0xB2)
ST7796_DIC = const(0xB4)
ST7796_PARCTRL = const(0xB6)
ST7796_GCTRL = const(0xB7)
ST7796_LCMCTRL = const(0xC0)
ST7796_PWR2 = const(0xC1)
ST7796_PWR3 = const(0xC2)
ST7796_VRHS = const(0xC3)
ST7796_VDVSET = const(0xC4)
ST7796_VCOM = const(0xC5)
ST7796_FRCTR2 = const(0xC6)
ST7796_PWCTRL1 = const(0xD0)
ST7796_PVGAMCTRL = const(0xE0)
ST7796_NVGAMCTRL = const(0xE1)
ST7796_DOCA = const(0xE8)
...
WIDTH_480 = [(320, 480,  0,  0),                          # added to support 480x320
             (480, 320,  0,  0),
             (320, 480,  0,  0),
             (480, 320,  0,  0)]
...
class ST7796():
    ...
    def __init__(self, spi, width, height, reset=None, dc=None,
        ...
        """
        #if height not in [320, 240] or width not in [480, 320, 240, 135]:                     # added to support 480x320
        #    raise ValueError("Unsupported display. 480x320, 320x240, 240x240 and 135x240 are supported.")
        if dc is None:
   ...
   #     time.sleep_ms(120)
    ...
    #    time.sleep_ms(120)
#        super().init()
  #      self._set_color_mode(COLOR_MODE_65K | COLOR_MODE_16BIT)      # Set Color Mode
   #     time.sleep_ms(10)
        #self.rotation(self._rotation)                                # Set Rotation
        self.inversion_mode(True)                                    # Set Inversion
      #  time.sleep_ms(10)
        self._write(ST7796_CSCON, bytes([0xC3]))                     # Enable extension command 2 partI
        time.sleep_ms(10)
        self._write(ST7796_CSCON, bytes([0x96]))                     # Enable extension command 2 partI
        time.sleep_ms(10)
        self._write(ST7796_MADCTL, bytes([0x48]))                    # X-Mirror, Top-Left to right-Buttom, RGB  
        time.sleep_ms(10)
        self._write(ST7796_COLMOD, bytes([0x55]))                    # Interface Pixel Format                                    
        time.sleep_ms(10)
        self._write(ST7796_DIC, bytes([0x01]))                       # 1-dot inversion
        time.sleep_ms(10)
        self._write(ST7796_PARCTRL, bytes([0x80, 0x02, 0x3B]))       # LCD Drive Line=8*(59+1)
        time.sleep_ms(10)
        self._write(ST7796_DOCA, bytes([0x40, 0x8A, 0x00, 0x00, 0x29, 0x19, 0xA5, 0x33]))
        time.sleep_ms(10)
        self._write(ST7796_PWR2, bytes([0x06]))                      # VAP(GVDD)=3.85+( vcom+vcom offset), VAN(GVCL)=-3.85+( vcom+vcom offset)
        time.sleep_ms(10)
        self._write(ST7796_PWR3, bytes([0xA7]))                      # Source driving current level=low, Gamma driving current level=High
        time.sleep_ms(10)
        self._write(ST7796_VCOM, bytes([0x18]))                      # VCOM=0.9
        time.sleep_ms(120)
        self._write(ST7796_PVGAMCTRL, bytes([0xF0, 0x09, 0x0B, 0x06, 0x04, 0x15, 0x2F, 0x54, 0x42, 0x3C, 0x17, 0x14, 0x18, 0x1B]))  
        time.sleep_ms(120)
        self._write(ST7796_NVGAMCTRL, bytes([0xE0, 0x09, 0x0B, 0x06, 0x04, 0x03, 0x2B, 0x43, 0x42, 0x3B, 0x16, 0x14, 0x17, 0x1B])) 
        time.sleep_ms(120)
        self._write(ST7796_CSCON, bytes([0x3C]))                     # Disable extension command 2 partI
        time.sleep_ms(10)
        self._write(ST7796_CSCON, bytes([0x69]))                     # Disable extension command 2 partII
        time.sleep_ms(10)
        if backlight is not None:                                    # Backlight ON if set
            backlight.value(1)
        time.sleep_ms(10)
   #     self._write(ST7796_NORON)                                    # NormalDisplay ModeOn
    #    time.sleep_ms(10)
        self.fill(0)
        time.sleep_ms(10)
    ...
    def _set_mem_access_mode(self, rotation, vert_mirror, horz_mirror, is_bgr):
        ...
        value = {
            ...
            1: ST7796_MADCTL_MX,
            2: ST7796_MADCTL_MY,
            3: ST7796_MADCTL_MX | ST7796_MADCTL_MY,
            4: ST7796_MADCTL_MV,
            5: ST7796_MADCTL_MV | ST7796_MADCTL_MX,
            6: ST7796_MADCTL_MV | ST7796_MADCTL_MY,
            7: ST7796_MADCTL_MV | ST7796_MADCTL_MX | ST7796_MADCTL_MY,
        }[rotation]

        if vert_mirror:
            value = ST7796_MADCTL_ML
        elif horz_mirror:
            value = ST7796_MADCTL_MH

        if is_bgr:
            value = ST7796_MADCTL_BGR
        self._write(ST7796_MADCTL, bytes([value]))
    ...
    def rotation(self, rotation):
        ...
        if self._display_width == 480:          # added to support 480
            table = WIDTH_320
            ...
            raise ValueError(
                "Unsupported display. 480x320, 320x240, 240x240 and 135x240 are supported."
            )
    ...
    def get_pixel(self, x, y):
        ...
        # Send read command
        self._write(0x2E)  # Read RAM command

        # Read the pixel data (assume 16-bit color)
        pixel_data = self._read_data(2)

        # Convert data to a 16-bit color value
        return (pixel_data[0] << 8) | pixel_data[1]
