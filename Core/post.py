...
def script():
    # Verbose boot: show info()-level messages only when enabled
    _vb = (regedit.read('Settings.Verbose_Boot') or 'false') == 'true'
    def _vi(msg, p='POST'):
        if _vb:
            core.info(msg, p=p)
    ...
    # "0" (or absent) means either first boot or a clean reboot/shutdown.
    startup_val = regedit.read("Settings.Startup") or "0"
    if startup_val == "1":
        _warn_unexpected_shutdown()

    # --- Clock setup ---
    # Sentinel 6: device crashed during first-boot clock calibration
    if startup_val == "6":
        regedit.save("Hardware.Clockable", "false")
        core.warn("Last boot failed during clock calibration. Clocking disabled.", p="POST")

    # Sentinel 7: device crashed while applying boot clock — disable auto-clock
    elif startup_val == "7":
    ...
    _ensure_log_dir()   # fix: guarantee /Nebula/Logs/ exists before first session
    ...
    # Sentinel "1": session is now active. A clean reboot writes "0" before
    # calling machine.reset(); if "1" survives to the next POST, it means
    # the previous session ended without a clean shutdown.
    regedit.save("Settings.Startup", "1")
    gc.collect()   # final cleanup — hand off to init with as clean a heap as possible
    return True