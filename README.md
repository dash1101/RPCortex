...
<p align="center">
  ...
  <a href="https://github.com/dash1101/RPCortex/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-GPL--3.0-fb923c"></a>
...
> **Work in progress.** It boots, sets itself up, logs in and gives you a full
...
what it found, and you answer five short questions — root password, owner, device
...
From there you have a shell that works. `ls` shows type, size, modified time and
name in columns. Arrows scroll your history and move the cursor mid-line; Ctrl+←
and Ctrl+→ jump by word; Tab completes commands, aliases and paths — and shows
you the candidates when there is more than one. Ctrl+C stops whatever is running,
whenever you press it.

You can pipe (`cat log | grep ERROR | wc`), chain (`wifi connect && ping 1.1.1.1`),
redirect (`sysinfo > boot.txt`), and quote things with spaces. `ll`, `dir`,
`more`, `del`, `free`, `gc` all do what you expect, because they did on v1.
...
`basename` `dirname` `echo`. All of them read a pipe when you give them one.
...
back off on an escalating delay; three misses return you to the username prompt
