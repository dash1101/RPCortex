    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int row = 0; row < DIGIT_H; row++) {
            for (int col = 0; col < DIGIT_W; col++) {
                ...
                int px = dx + col;
                // No canvas read per pixel: whether the fill is behind this
                // column is arithmetic, and get() on every one of the fifty-five
                // would be the expensive way to ask a question already answered.
                c.pixel(px, dy + row, px < ix + fill_w ? 0 : 1);
            }
        }
        dx += DIGIT_W + 1;
    }
